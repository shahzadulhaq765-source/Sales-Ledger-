...
function selectOptions(){
 ...
 if($("#schemeProduct"))$("#schemeProduct").innerHTML='<option value="">Select product</option>'+db.products.map(p=>`<option value="${esc(p.code)}">${esc(p.code)} — ${esc(p.name)}</option>`).join("");
...
async function invoiceCanvas(inv){
 ...
 try{const stamp=await loadImage("assets/suh-blue-stamp.png"),sig=await loadImage("assets/authorized-signature.png");const sw=155,sh=155,sx=945,sy=1450;x.drawImage(stamp,sx,sy,sw,sh);x.drawImage(sig,sx-30,sy+35,220,120)}catch(e){}
...
async function downloadXlsx(name,rows){const escXml=v=>String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");const col=n=>{let out="";for(let x=n+1;x;x=Math.floor((x-1)/26))out=String.fromCharCode(65+(x-1)%26)+out;return out};let sheet='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>';rows.forEach((r,ri)=>{sheet+=`<row r="${ri+1}">`;r.forEach((v,ci)=>{const ref=col(ci)+(ri+1);if(typeof v==="number"&&Number.isFinite(v))sheet+=`<c r="${ref}"><v>${v}</v></c>`;else sheet+=`<c r="${ref}" t="inlineStr"><is><t>${escXml(v)}</t></is></c>`});sheet+='</row>'});sheet+='</sheetData></worksheet>';const zip=new JSZip();zip.file('[Content_Types].xml','<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/></Types>');zip.folder('_rels').file('.rels','<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>');zip.folder('xl').file('workbook.xml','<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Data" sheetId="1" r:id="rId1"/></sheets></workbook>');zip.folder('xl').folder('_rels').file('workbook.xml.rels','<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>');zip.folder('xl').folder('worksheets').file('sheet1.xml',sheet);const blob=await zip.generateAsync({type:'blob',mimeType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name.endsWith('.xlsx')?name:name+'.xlsx';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
function updateSchemePreview(){if(!$("#schemeProduct"))return;const p=db.products.find(x=>x.code===$("#schemeProduct").value),a=Number($("#schemeAmount").value||0);$("#schemeTp").value=p?Number(p.tpPrice||0):"";$("#schemeNet").value=p?Math.max(0,Number(p.tpPrice||0)-a).toFixed(2):""}
...
if($("#saveScheme"))$("#saveScheme").onclick=()=>{const customerCode=$("#schemeCustomer").value,productCode=$("#schemeProduct").value,p=db.products.find(x=>x.code===productCode),schemeAmount=Number($("#schemeAmount").value||0),validFrom=$("#schemeFrom").value,validTo=$("#schemeTo").value;if(!customerCode||!p||!validFrom||!validTo)return toast("Customer, product and validity dates required");db.schemes.push({id:id(),customerCode,productCode,tpRate:Number(p.tpPrice||0),schemeAmount,specialPrice:Math.max(0,Number(p.tpPrice||0)-schemeAmount),validFrom,validTo,createdAt:new Date().toISOString()});persist();renderSchemes();toast("Scheme saved")};
...
renderAll();
if(!db.products.length)loadBundledProducts(false).then(()=>renderAll());
