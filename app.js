...
function renderProducts(){
 ...
 $("#productList").innerHTML=db.products.filter(p=>(p.code+" "+p.name+" "+(p.brand||"")).toLowerCase().includes(q)).map(p=>`<div class="list-row"><div><b>${esc(p.code)} — ${esc(p.name)}</b><div class="muted">${esc(p.brand||"")} • ${esc(p.packing||"")} • ${num(p.cartonWeight)} kg/carton</div></div><div class="right"><b>${money(p.rate)}/ton</b><div class="muted">${(p.priceHistory||[]).length} price change(s)</div><div class="mini-actions"><button onclick="editProduct('${esc(p.code)}')">Edit</button></div></div></div>`).join("")||'<p class="muted">No products.</p>';
}
...
function renderLines(){
 $("#invoiceLines").innerHTML=lines.map((l,i)=>`<tr><td>${esc(l.code)}</td><td>${esc(l.name)}</td><td>${esc(l.packing||"")}</td><td>${l.cartons}</td><td>${num(l.cartonWeight)}</td><td>${num(l.kg)}</td><td>${num(l.tonnage)}</td><td>${money(l.rate)}</td><td>${money(l.amount)}</td><td><button class="remove" onclick="removeLine(${i})">×</button></td></tr>`).join("");
...
$("#saveProduct").onclick=()=>{const old=$("#prodEdit").value,code=$("#prodCode").value.trim(),name=$("#prodName").value.trim(),rate=Number($("#prodRate").value||0);if(!code||!name)return toast("Product code and name required");if(!old&&db.products.some(p=>p.code===code))return toast("Product code already exists");const base={code,name,brand:$("#prodBrand").value.trim(),packing:$("#prodPacking").value.trim(),cartonWeight:Number($("#prodWeight").value||0),rate};if(old){const i=db.products.findIndex(p=>p.code===old),prev=db.products[i];const hist=[...(prev.priceHistory||[])];if(prev.rate!==rate)hist.push({date:today(),oldRate:prev.rate,newRate:rate});db.products[i]={...prev,...base,priceHistory:hist}}else db.products.push({...base,priceHistory:[]});clearProductForm();persist();toast("Product saved")};
function clearProductForm(){["#prodEdit","#prodCode","#prodName","#prodBrand","#prodPacking","#prodWeight","#prodRate"].forEach(s=>$(s).value="");$("#cancelProductEdit").classList.add("hidden")}
window.editProduct=code=>{const p=db.products.find(x=>x.code===code);if(!p)return;$("#prodEdit").value=p.code;$("#prodCode").value=p.code;$("#prodName").value=p.name;$("#prodBrand").value=p.brand||"";$("#prodPacking").value=p.packing||"";$("#prodWeight").value=p.cartonWeight;$("#prodRate").value=p.rate;$("#cancelProductEdit").classList.remove("hidden");switchTab("products")};$("#cancelProductEdit").onclick=clearProductForm;

$("#lineProduct").onchange=()=>{const p=db.products.find(x=>x.code===$("#lineProduct").value);$("#lineRate").value=p?p.rate:"";previewLine()};
$("#lineCartons").oninput=previewLine;$("#lineRate").oninput=previewLine;
function previewLine(){const p=db.products.find(x=>x.code===$("#lineProduct").value),c=Number($("#lineCartons").value||0),r=Number($("#lineRate").value||0);if(!p||!c){$("#linePreview").textContent="";return}const kg=c*p.cartonWeight,t=kg/1000;$("#linePreview").textContent=`${c} cartons × ${p.cartonWeight} kg = ${num(kg)} kg = ${num(t)} tons • Amount ${money(t*r)}`}
$("#addLine").onclick=()=>{const p=db.products.find(x=>x.code===$("#lineProduct").value),cartons=Number($("#lineCartons").value||0),rate=Number($("#lineRate").value||0);if(!p||cartons<=0)return toast("Select product and enter cartons");const kg=cartons*p.cartonWeight,tonnage=kg/1000;lines.push({code:p.code,name:p.name,packing:p.packing||"",cartons,cartonWeight:p.cartonWeight,kg,tonnage,rate,amount:tonnage*rate});$("#lineCartons").value="";previewLine();renderLines()};
...
$("#exportCsv").onclick=()=>{const rows=[["Invoice No","Date","Customer Code","Customer Name","Status","Product Code","Product Name","Cartons","Carton Weight KG","Tonnage","Rate Per Ton","Amount"]];db.invoices.forEach(i=>i.lines.forEach(l=>rows.push([i.invoiceNo,i.date,i.customerCode,cName(i.customerCode),i.status,l.code,l.name,l.cartons,l.cartonWeight,l.tonnage,l.rate,l.amount])));download("SUH-sales-"+today()+".csv",rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(",")).join("\n"),"text/csv")};
